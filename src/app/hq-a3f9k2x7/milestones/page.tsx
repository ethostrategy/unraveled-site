...
const OVERVIEW: { name: string; color: string; work: [number, number]; yearOnly?: boolean; ms: { t: string; q: number; cont?: boolean; detail?: boolean; desc?: string; gated?: boolean }[] }[] = [
  ...
  { name: "Intelligence", color: "#9a7fe0", work: [2, 13], ms: [
    ...
    { t: "App V1 (profiles + assessments)", q: 3.71, desc: "App V1 launches Dec 5, 2026 (the Unraveled founding day) — the FIRST product to market, ahead of the cards. Web + mobile: Unraveled profiles + Two Truths, plus any other assessments (Anchor Styles, Blueprint, Love Dialects) that clear SME review by launch, and the QR deck-play + ecosystem-unlock system so the in-deck QR codes work the moment Between Us drops. Lightweight personalization so the cohort loop starts early; the full algorithm comes with App V2. Built by Madhuri on Base44 (an AI app builder), so V1 ships without the Founding AI Engineer — that hire stays deferred to V2 hardening." },
  ...
  { name: "Brand/Media", color: "#e273ac", work: [0, 15], ms: [
    { t: "Rebrand", q: 0.5, desc: "Brand refresh — visual identity + positioning locked (2026 Q1)." }, { t: "Website", q: 1.5, desc: "New marketing / waitlist site live (2026 Q2)." }, { t: "Instagram", q: 3.1 }, { t: "Newsletter", q: 3.15, desc: "Beehiiv newsletter ('The Unraveled Letters') starts ~early Oct 2026 to build an owned email list BEFORE the Dec 5 app launch + the early-2027 card drop. Format: 3-2-1 — 3 ways to build connection this week, 2 things to sit with (reply → we feature you on Instagram), 1 help/safety resource — plus founder updates + an early-access CTA. Original + framework-derived, never the actual card prompts. The list you grow now is who you launch the app + cards to." }, { t: "Podcast + YouTube", q: 4.1, desc: "Podcast + YouTube: filming can't start until late Nov 2026, so the podcast can't power the Dec 5 app launch or the early-2027 card drop (TikTok + Instagram carry those). First episodes publish ~Jan 2027; podcast clips then become the marketing engine through 2027." }, { t: "TikTok", q: 3.5, desc: "TikTok launches with the campaign (~Nov 2026): the youth channel that drives the Dec 5 app launch and the early-2027 Between Us drop, building grassroots cred (street 'answer this card' videos, launch hype)." }, { t: "Threads + Reddit", q: 6.3 }, { t: "Sports/fitness partnerships", q: 9.8, desc: "Sports + fitness brand partnerships (~mid 2028), a growth-phase play once grassroots cool is established." }, { t: "Merch", q: 7.0, desc: "First 'built, not found' merch drop (~late 2027): simple apparel + goods, not a complex supply chain, a cheap community + revenue moment riding the card-game audience. Bigger co-branded merch scales with the sports/fitness partnerships in 2028." },
  ] },
  { name: "B2C Products", color: "#c768c6", work: [2, 14], ms: [
    ...
    { t: "Between Us launch", q: 4.3, desc: "Between Us launches with Real Talk + Sweet Talk a few weeks before Valentine's Day (~early Feb 2027): the first physical product and first hardware revenue, following the app so the in-deck QR activates digital play + an ecosystem unlock on day one. Decided (Sep 2026): Real Talk + Sweet Talk lead, sold individually or as a dual package; Self Talk and the spicy Dirty Talk edition follow. The pre-launch campaign kicks off at the Dec 5 app launch." },
