...
const OVERVIEW: { name: string; color: string; work: [number, number]; ms: { t: string; q: number; cont?: boolean; detail?: boolean; desc?: string; gated?: boolean }[] }[] = [
  { name: "Framework", color: "#6f8fd8", work: [1, 13], ms: [
    ...
    { t: "V1 drafted", q: 2.49, desc: "Madhuri drafts V1: the 10 blocks + assessments written up as the framework's first white-paper draft (finalized ~Aug 15, 2026). Drafted by her, not the intern." },
    { t: "V2 (clinical review)", q: 2.9, detail: true, desc: "The clinical pre-review (a physician with a Harvard MPH, UC Davis OBGYN resident) critiques V1; revised into V2." },
    { t: "Reviewed by Dr. Burke", q: 3.5, desc: "Dr. Nadine Burke reviews V2 (the framework AND assessments; whatever clears sets App V1's scope); revised into V3, the launch-ready framework. Reached via the Berkeley / UC network, no equity." },
    { t: "White paper (V3)", q: 5.5, desc: "Publish V3 as a citable preprint (PsyArXiv/OSF) + on the site: the reviewed, launch-ready framework white paper (~May 2027)." },
    { t: "Journal submission", q: 7.5, desc: "Submit the larger writeup/artifact to a peer-reviewed journal via Berkeley psych connections (undergrad network), not the MBA. Review takes ~6-18 months." },
    { t: "Validation study", q: 10, desc: "First empirical study on real, consented app data (block independence, longitudinal), the source of genuine psychometric credibility. Feeds a data-informed V4." },
    { t: "V4 (data-informed)", q: 11.5, desc: "Revise the framework from validation data + reviewer critique: the data-informed edition, versioned explicitly." },
    ...
    <main className="relative min-h-screen text-white">
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mt-6 flex flex-wrap gap-2 text-[13px]">
          ...
          <a href="/hq-a3f9k2x7/board" className="rounded-full border border-white/10 px-3.5 py-1 text-white/60 transition hover:text-white">This week</a>
              ...
              <div className="pointer-events-none absolute inset-y-0 z-0" style={{ left: 104, right: 0 }}>
                ...
                {nowInView && (
                  <div className="absolute z-10 flex flex-col items-center" style={{ left: `${nowLeft}%`, top: -13, transform: "translateX(-50%)" }}>
                    ...
                    <span className="h-2.5 w-px bg-[#e273ac]" />
                  </div>
                )}
              </div>
