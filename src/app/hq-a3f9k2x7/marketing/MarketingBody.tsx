...
/**
 * The marketing operating system content — the funnel, content pillars/types,
 * the content engine, the social playbook, principles, and the cost picture +
 * budget calculator. Rendered inside the Strategy tab (Foundation → Marketing);
 * the /marketing route redirects here.
 */
...
const CONTENT_PRINCIPLES: { icon: string; t: string; d: string; flow?: string[]; loop?: boolean; tactics?: string[] }[] = [
  ...
  return (
    <div>
      ...
      <p className="mt-8 max-w-2xl text-[13.5px] leading-relaxed text-white/70">
        The marketing operating system, the <span className="text-white/90">how</span>. Each product carries its own funnel in its Strategy vertical; this is the shared playbook behind all of them.
      </p>
      ...
      <p className="mt-6 max-w-2xl text-[12.5px] leading-snug text-white/55">
        Ad spend is the variable one. It&rsquo;s the funnel as math: <span className="text-white/80">impressions × CTR = clicks; clicks × CVR = conversions</span>; cost comes from CPC (per click) or CPM (per 1,000 impressions). Drag the assumptions to work backward from a spend to a cost per conversion.
      </p>
      <BudgetCalc />
